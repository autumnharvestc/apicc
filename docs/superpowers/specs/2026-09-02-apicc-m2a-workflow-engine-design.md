...
3. 无 missing 引用
4. **单起始覆盖**：至少一个起始节点（入度 0）可达全部非孤立节点；不满足 → error。多起始工作流（含多个独立流程）无法通过启用校验属有意护栏——独立流程应拆分为独立工作流（执行器仍支持多起始顺序运行草稿，便于迁移）
5. **非空编排**（2026-09-03 增补）：工作流至少一个节点；0 节点 → error「工作流没有任何节点，无法启用」——空流没有可执行语义，禁止启用
