...
describe("viewSwitch 模块禁用语义（M8 模块化）", () => {
  it("模块清单：7 模块固定顺序（rail 渲染顺序契约）", () => {
    expect(SWITCH_VIEWS).toEqual(["api", "run", "wf", "stress", "envs", "import", "plugins"]);
    expect(API_SUB_VIEWS).toEqual(["debug", "design", "cases"]);
  });

  it("在线模式激活：全部模块禁用（含 plugins——内容区让位 OnlineApiEditor）", () => {
  ...
  it("未打开工作区（非在线）：工作区级模块禁用，plugins（管理类视图，裁定①）恒可用", () => {
