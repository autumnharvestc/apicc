...
describe("在线接口编辑（步骤 1①②：VIEWER 只读 vs EDITOR 可编辑，坏数据进 problems）", () => {
  ...
  it("canEdit：工作区 VIEWER → 恒只读；EDITOR + 项目 VIEWER/NONE 覆盖 → 该项目只读", async () => {
    const { store } = await opened();
    expect(store.canEdit(API_PATH)).toBe(true);
    // 项目级 ACL 覆盖：tree.projects 里示例项目 myRole=EDITOR（继承 OWNER 工作区），改覆盖为 VIEWER
    store.projects = [{ id: "p-online-1", name: "示例项目", myRole: "VIEWER" }];
    expect(store.canEdit(API_PATH)).toBe(false);
    store.projects = [{ id: "p-online-1", name: "示例项目", myRole: "NONE" }];
    expect(store.canEdit(API_PATH)).toBe(false);
    // 工作区级 VIEWER：一切只读
